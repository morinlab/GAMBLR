...
#'
prettyOncoplot = function (maftools_obj, onco_matrix_path, genes, include_noncoding = NULL, 
                                keepGeneOrder = FALSE, keepSampleOrder = TRUE, highlightHotspots = FALSE, 
                                these_samples_metadata, metadataColumns, numericMetadataColumns, 
                                expressionColumns = c(), numericMetadataMax, sortByColumns, 
                                arrange_descending = FALSE, removeNonMutated = FALSE, minMutationPercent, 
                                mutAlpha = 1, recycleOncomatrix = FALSE, splitColumnName, 
                                splitGeneGroups, showTumorSampleBarcode = FALSE, groupNames, 
                                hide_annotations, annotate_specific_genes = FALSE, this_forest_object = NULL, 
                                custom_colours = NULL, hideTopBarplot = TRUE, tally_all_mutations = FALSE, 
                                tally_all_mutations_max = 1000, hideSideBarplot = FALSE, 
                                box_col = NA, annoAlpha = 1, legend_direction = "horizontal", 
                                ylim = NULL, legend_position = "bottom", legend_row = 3, 
                                legend_col = 3, metadataBarHeight = 1.5, metadataBarFontsize = 5, 
                                legendFontSize = 10, fontSizeGene = 6, annotation_row = 2, 
                                annotation_col = 1, verbose = FALSE) 
{
  ...
  if (!recycleOncomatrix & missing(onco_matrix_path)) {
    ...
    if (any(!maf_patients %in% patients)) {
      ...
      patients = maf_patients[which(maf_patients %in% 
                                      patients)]
      n_drop = length(extra)
    ...
    if (missing(genes)) {
      genes = maftools::getGeneSummary(x = maftools_obj)[order(MutatedSamples, 
      ...
      genes = genes[fractMutated * 100 >= minMutationPercent, 
                    Hugo_Symbol]
      lg = length(genes)
      message(paste("creating oncomatrix with", lg, "genes"))
      om = maftools:::createOncoMatrix(m = maftools_obj, 
                                       g = genes, add_missing = TRUE)
      mat_origin = om$oncoMatrix
      tsbs = levels(maftools:::getSampleSummary(x = maftools_obj)[, 
      ...
      write.table(mat_origin, file = onco_matrix_path, 
                  quote = F, sep = "\t")
    }
    else {
  ...
  if (!missing(numericMetadataColumns)) {
    message(paste0("The column(s) ", numericMetadataColumns, 
                   " specified both in metadata and numeric metadata. Plotting as numeric values..."))
    metadataColumns = metadataColumns[!metadataColumns %in% 
                                        numericMetadataColumns]
  }
  patients = pull(these_samples_metadata, sample_id)
  old_style_mat = read.table(onco_matrix_path, sep = "\t", 
                             stringsAsFactors = FALSE)
  mat = read.table(onco_matrix_path, sep = "\t", header = TRUE, 
                   check.names = FALSE, row.names = 1, fill = TRUE, stringsAsFactors = F, 
                   na.strings = c("NA", ""))
  ...
  if (length(include_noncoding) > 0) {
    ...
    for (gene in names(include_noncoding)) {
      for (this_vc in unname(include_noncoding[[gene]])) {
        ...
        these_samples = dplyr::filter(maftools_obj@maf.silent, 
                                      Hugo_Symbol == gene & Variant_Classification == 
                                        this_vc) %>% dplyr::select(Tumor_Sample_Barcode, 
                                                                   Variant_Classification) %>% unique() %>% pull(Tumor_Sample_Barcode)
  ...
  alter_fun = list(background = function(x, y, w, h) {
    ...
    grid.rect(x, y, w - unit(spacing, "pt"), 0.75 * h * 
                height_scaling, gp = gpar(fill = col["Nonsense_Mutation"], 
                                          col = box_col))
    ...
    grid.rect(x, y, w - unit(spacing, "pt"), (height_scaling/5) * 
                h, gp = gpar(fill = "white", col = box_col))
  }, Silent = function(x, y, w, h) {
    grid.rect(x, y, w - unit(spacing, "pt"), (height_scaling/5) * 
                h, gp = gpar(fill = col["Silent"], col = box_col))
  })
  ...
  all_gambl_colours = get_gambl_colours()
  for (column in metadataColumns) {
    these_samples_metadata[[column]] = factor(these_samples_metadata[[column]], 
                                              levels = unique(these_samples_metadata[[column]]))
    options = these_samples_metadata %>% arrange(column) %>% 
      dplyr::filter(!is.na(column)) %>% pull(column) %>% 
      unique()
    ...
    if (column == "sex") {
    ...
    else if (sum(levels(options) %in% names(clinical_colours)) == 
             length(levels(options))) {
      if (verbose) {
    ...
    else if (("positive" %in% options | "POS" %in% options | 
              "yes" %in% options) & length(options) < 4) {
      if (verbose) {
        print("using pos_neg")
      }
      these = get_gambl_colours("pos_neg", alpha = annoAlpha)
    ...
    else if (sum(levels(options) %in% names(all_gambl_colours)) == 
             length(levels(options))) {
      if (verbose) {
        message(paste("found colours for", column, "in all_gambl_colours"))
      }
      ...
      these = blood_cols[sample(c(1:length(blood_cols)), 
                                size = length(levels(options)))]
      names(these) = levels(options)
      if (!"NA" %in% names(these)) {
        these = c(these, `NA` = "white")
      }
  ...
  if (highlightHotspots) {
    hot_samples = dplyr::filter(maftools_obj@data, hot_spot == 
                                  TRUE & Hugo_Symbol %in% genes) %>% dplyr::select(Hugo_Symbol, 
    ...
    for (i in colnames(mat)) {
      mat[genes, i][!is.na(hot_mat[genes, i])] = paste0(mat[genes, 
                                                            i][!is.na(hot_mat[genes, i])], ";", hot_mat[genes, 
                                                                                                        i][!is.na(hot_mat[genes, i])])
    }
  ...
  if (!missing(numericMetadataColumns)) {
    metadata_df = dplyr::filter(these_samples_metadata, 
                                sample_id %in% patients_kept) %>% column_to_rownames("sample_id") %>% 
  ...
  col_fun = circlize::colorRamp2(c(0, 0.5, 1), c("blue", "white", 
                                                 "red"))
  for (exp in expressionColumns) {
    colours[[exp]] = col_fun
  }
  ...
  heatmap_legend_param = list(title = "Alterations", at = c("RNA", 
                              ...
                              labels = c("RNA", "3'UTR", "Nonsense Mutation", "Splice Site", 
                                         ...
                                         "Silent", "Hotspot"), nrow = annotation_row, ncol = annotation_col, 
                              legend_direction = legend_direction, labels_gp = gpar(fontsize = legendFontSize))
  if (hideTopBarplot) {
    top_annotation = NULL
  }
  else {
    tally_mutations = maftools_obj@data %>% dplyr::filter(Tumor_Sample_Barcode %in% 
    ...
    enrichment_label = mat[intersect(genes, genes_kept), 
      ...
      select(gene) %>% left_join(this_forest_object$fisher %>% 
                                   select(gene, estimate, q.value)) %>% mutate(`Enriched in` = case_when(estimate == 
                                                                                                           ...
                                                                                                           "-Inf" & q.value <= 0.1 ~ these_comparisons[2], 
                                                                                                         is.na(estimate) ~ "NA", estimate <= 1 & q.value <= 
                                                                                                           0.1 ~ these_comparisons[2], estimate > 1 & q.value <= 
                                                                                                           0.1 ~ these_comparisons[1], TRUE ~ "Both")) %>% 
      pull("Enriched in")
    right_annotation = rowAnnotation(` ` = enrichment_label, 
                                     col = list(` ` = c(get_gambl_colours()[these_comparisons], 
                                                        Both = "#ACADAF", `NA` = "#000000")), simple_anno_size = unit(metadataBarHeight, 
  ...
  ch = ComplexHeatmap::oncoPrint(mat[intersect(genes, genes_kept), 
                                     patients_kept], alter_fun = alter_fun, top_annotation = top_annotation, 
                                 right_annotation = right_annotation, col = col, row_order = gene_order, 
                                 column_order = column_order, column_labels = NULL, show_column_names = showTumorSampleBarcode, 
                                 column_split = column_split, column_title = column_title, 
                                 row_title = NULL, row_split = row_split[intersect(genes, 
                                                                                   genes_kept)], heatmap_legend_param = heatmap_legend_param, 
                                 row_names_gp = gpar(fontsize = fontSizeGene), pct_gp = gpar(fontsize = fontSizeGene), 
                                 bottom_annotation = ComplexHeatmap::HeatmapAnnotation(df = metadata_df, 
                                                                                       show_legend = show_legend, col = colours, simple_anno_size = unit(metadataBarHeight, 
                                                                                                                                                         "mm"), gap = unit(0.25 * metadataBarHeight, 
                                                                                                                                                                           "mm"), annotation_name_gp = gpar(fontsize = metadataBarFontsize), 
                                                                                       annotation_legend_param = list(nrow = legend_row, 
                                                                                                                      col_fun = col_fun, ncol = legend_col, direction = legend_direction, 
                                                                                                                      labels_gp = gpar(fontsize = legendFontSize))))
