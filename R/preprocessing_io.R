...
#'
#' @param targ_df provide a data frame with all file details
...
#' @param target_path Path to targ_df
#'
#' @return
#' @export
#'
...
#' Title
#'
#' @param tool Name of tool to be included in results
  ...
  if(tool == "battenberg_ploidy"){
    ...
    parse_batt = function(batt_file){
      batt_data = batt_file %>%
        purrr::map(read_tsv) %>%
        purrr::reduce(rbind) %>%
        dplyr::rename(battenberg_cellularity =c ellularity, battenberg_ploidy = ploidy,battenberg_psi = psi)
      return(batt_data)
    }
    ...
    for(unix_group in unix_groups){
      ...
      for(genome_build in genome_builds){
        ...
        files = fetch_output_files(build = genome_build, unix_group = unix_group, base_path = paste0(unix_group, "/battenberg_current"), results_dir = "02-battenberg", tool = "battenberg_ploidy")
...
#'
#' @param bedpe_paths Path to bed files
...
#' This is a helper function that is not meant to be used routinely
#'
#' @param file_df Input df (bedpe)
#' @param out_dir Ouput directory
#' @param group Unix group
#' @param genome_build Version of genome reference build
...
process_all_manta_bedpe = function(file_df,
  ...
  process_manta = function(bedpe_file,
                           liftover_to_hg19 = FALSE,
                           liftover_to_hg38 = FALSE,
                           only_return_missing = FALSE,
                           projection="grch37"){
  ...
  if(projection_build == "grch37"){
    ...
    if(genome_build == "hg38"){
      ...
      not_hg38_files = dplyr::filter(file_df, genome_build != "hg38" & unix_group == group) %>% 
        pull(file_path)
      bed_data_lifted =n ot_hg38_files %>%
...
#' Title
#'
#' @param tool_name Name of tool e.g manta
...
#' @param results_dir Directory for storing results, default is 99-outputs
#' @param seq_type Sequence type, default is genome
#' @param build Reference genome build, default is hg38
#' @param search_pattern Search pattern for fiel extension, default is cellularity_ploidy.txt
#'
...
#' Title
#'
#' @param tool_results_path Path to tool results
#' @param search_pattern Search pattern for locating files
#' @param genome_build Reference genome build
#' @param seq_type Sequence type, default is genome
#' @param unix_group Default, gambl
#' @param tool_name name of tool, e.g manta
#'
#' @return
#' @export
...
#' Function for reading MAF files
#'
...
#' @param return_df Option to return data frame, default is FALSE
#'
#' @return
#' @export
#'
...
#' Function for updates the database by appending to the gambl_files table. Use with caution!
#'
...
#' @param is_production Default is YES
#'
...
#' @param col_names Column names, if none entered, function will impose col names
#' @param col_types Column type, needed if col_names is entered
#' @param standard_bed Boolean statement, defaults to FALSE
#'
