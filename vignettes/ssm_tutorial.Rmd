...
#peek return
paged_table(dlbcl_metadata, options = list(rows.print = 5, cols.print = 5))
```
...
#peek return
paged_table(this_sample_ssm, options = list(rows.print = 5, cols.print = 5))
```
...
#peek return
paged_table(these_samples_ssm, options = list(rows.print = 5, cols.print = 5))
```
...
these_patients_ssm = get_ssm_by_patients(these_patient_ids = these_patients,
                                         ...
                                         subset_from_merge = TRUE)
#peek return
paged_table(these_patients_ssm, options = list(rows.print = 5, cols.print = 5))
```
...
#peek return
paged_table(myc_ssm, options = list(rows.print = 5, cols.print = 5))
```
...
ashm_ssm = get_ssm_by_regions(regions_bed = ashm_regions_bed,
                              projection = "grch37",
                              seq_type = "genome",
                              mode = "slms-3",
                              min_read_support = 3,
...
#peek return
paged_table(ashm_ssm, options = list(rows.print = 5, cols.print = 5))
```

### Coding SSM
Lastly, another way to retrieve SSMs is to call `get_coding_ssm`. This function reads in all coding SSMs from the GAMBL database. Convenient filtering parameters are included in this function for easy and straight-forward subset to specific pathologist, cohorts, and sample IDs. If these parameters are not called, coding SSMs will be returned for all samples. In the example bellow, we are using a metadata subset (DLBCL cases) for retrieving coding ssm.
...
#peek return
paged_table(all_coding_ssm, options = list(rows.print = 5, cols.print = 5))
```
...
#peek return
paged_table(deblacklisted_ssm, options = list(rows.print = 5, cols.print = 5))
```
...
#peek return
paged_table(driver_ssm, options = list(rows.print = 5, cols.print = 5))
```
...
#peek return
paged_table(this_sample_ssm_hot, options = list(rows.print = 5, cols.print = 5))
```
...
#peek return
paged_table(this_sample_ssm_hot_rev, options = list(rows.print = 5, cols.print = 5))
```
...
bl_genes = c("MYC", "ID3", "TP53", "ARID1A", "FBXO11", 
