...
WARNING! This function have been updated to read from the deblacklisted and merged maf (instead of using the database). Thus, this chunk is rather slow and consumes a lot of memory. Should be updated to read from demo-dataset, when available. Alternativly, this chunk could be removed untiul then.

```{r mutation_totals, out.width = "75%", warning = FALSE}
# Define seqtype and genome projection for to be retreived MAF.
seq_type = "genome"
projection = "grch37"

#set path to merged and deblacklsited MAF.
...
# Read MAF. 
hg19_maf = fread_maf(full_maf_path)

# Subset mutations by sample
mutation_counts = hg19_maf %>%
  group_by(Tumor_Sample_Barcode) %>%
  tally()

# Print head of mutation counts dataframe.
print(head(mutation_counts))

# Plot histogram with mutation counts .
mutation_counts %>%
  ggplot() +
...
Retrieve somatic mutations (MYC) for a given cohort and visualization using MAFtools.
...
maf_obj = read.maf(maf_data)
lollipopPlot(maf_obj, gene = "MYC")
```
...
```{r pretty_oncoplot, warning = FALSE, message = FALSE, out.width = "100%", fig.dim = c(8,3)}
...
Sometimes it can be useful to specify the order of how a specific metadata column is represented. For demonstration, in the prettyOncoplot bellow the pathology column in the metadata is reordered so that DLBCL is shown before BL. This is achieved by first converting the desired metadata column into a factor and then specifying the order of those factors.

```{r pretty_oncoplot_reordered, warning = FALSE, message = FALSE, out.width = "100%", fig.dim = c(8,3)}
...
maf_metadata$pathology = factor(maf_metadata$pathology, levels = c("DLBCL", "BL", "B-ALL", "CLL", "COMFL", "DLBCL-BL-like", "FL", "HGBL", "MCL", "PBL", "SCBC", "UNSPECIFIED"))
...
some_meta = left_join(some_meta, collated) %>%
    dplyr::filter(genetic_subgroup != "0") #remove genetic subgroups eqaul to 0.
...
Construct a sample-level, genome-wide ideogram to visualize CN segments. Optional argument to show SVs (lollipops) and annotate ideogram with gene(s). Plotting function calls get_cn_segments and assign_cn_to_ssm to retrieve relevant data. Thus, parameters available for such functions are also at the users disposal for sub-setting variants to coding regions and flatfile/augmented maf options. In addition, the user can also annotate LOH regions and SV counts per chromosome as such:

```{r ideogram, dpi = 150, fig.retina = 2, warnings = FALSE, echo = FALSE, messages = FALSE, fig.height = 12, fig.width = 17}
# Define sample to be plotted
sample_id = "HTMCP-01-06-00422-01A-01D"

# Set gene for gene_annotation argument (calls gene_to_region) in plotting function.
gene_annotation = "MYC"

# Construct plot.
fancy_ideogram(this_sample = sample_id, 
...
Another useful plot for visualizing sample-level CN information is the fancy_cnvlohbar. Similarly to fancy_ideogram, this function takes one required parameter (sample id), calls get_cn_segments and assign_cn_ssm to retrieve data for plotting. The plot highlights the number of CN states for any given sample (first y-axis), together with n bases included in each CN state (second y-axis).

```{r dpi = 150, fig.retina = 2, warnings = FALSE, echo = FALSE, messages = FALSE, fig.height = 7, fig.width = 7}
# Define sample to be plotted.
sample_id = "HTMCP-01-06-00422-01A-01D"

# State chromosomes to be included for the selected sample.
chromosomes = paste0("chr", c(1:22))

# Construct plot using above defined parameters.
fancy_cnlohbar(this_sample = sample_id,
               chr_select = chromosomes,
               include_cn2 = TRUE, 
               coding_only = FALSE)
```

Interested in how many variants are present in a given sample? One quick way to overview such information would be to call fancy_svbar on a given sample id. This function returns a bar plot visualizing SVs. A variety of sub-setting options are also available. Such as chromosome selection and SV types to be included in plot. 

```{r dpi = 150, fig.retina = 2, warnings = FALSE, echo = FALSE, messages = FALSE, fig.height = 7 fig.width = 7}
# Set sample ID for plotting.
sample_id = "HTMCP-01-06-00422-01A-01D"

# Define plot subtitle.
plot_sub = "Structural Variant Subtype Distribution"

# State chromosomes to be included for the selected sample.
chromosomes = paste0("chr", c(1:22))

# State what SV sub type are to be plotted.
svs = c("DEL", "INS")

# Construct plot using above defined parameters.
fancy_svbar(this_sample = sample_id,
            plot_subtitle = plot_sub,
            chr_select = chromosomes,
            variant_select = svs,
            coding_only = FALSE)
```

Curious to see how SNVs are distributed across specific chromosomes? For this purpose fancy_snv_chrdistplot was developed. This function takes a sample ID and retrieves SNV information (using assign_cn_to_ssm) and plots variants in an ideogram dependent manner. Similarly to other fanxy_x_plot functions, a selection of parameters are available for data sub-setting and plot customization. For example, bu using the include_dnp, DNP counts will be added to each bar. 
...
# Define plot subtitle.
plot_sub = "SNV Distribution Per Chromosome"

# State chromosomes to be included for the selected sample.
chromosomes = paste0("chr", c(1:5))

# Construct plot using above defined parameters.
fancy_snv_chrdistplot(this_sample = sample_id, 
                      plot_subtitle = plot_sub,
                      chr_select = chromosomes,
                      include_dnp = FALSE,
                      coding_only = FALSE)
...
Sometimes it can be useful to overview SV distribution across the complete genomic landscape. To do so, one could call fancy_sv_chrdistplot with sample ID. Optional arguments for sub-setting your data are avaialble (see parameter descriptions for more information)
...
# Define plot subtitle.
plot_sub = "Structural Variant Distribution Per Chromosome"

# State chromosomes to be included for the selected sample.
chromosomes = paste0("chr", c(1:22))

# Construct plot using above defined parameters.
fancy_sv_chrdistplot(this_sample = sample_id, 
                     plot_subtitle =  plot_sub,
                     chr_select = chromosomes, 
                     coding_only = FALSE)
```

Another useful way to visualize SV distributions is to call fancy_vplot to generate a violin plot showing SV size distributions (sample-level) across selected contigs (per default chr1-22). Optional parameters to sub-set data to include only coding regions are also available for this function.
...
# Define plot subtitle.
plot_sub = "Structural Variant Size Distribution (chr1 - chr22)"

# State chromosomes to be included for the selected sample.
chromosomes = paste0("chr", c(1:22))

# Construct plot using above defined parameters.
fancy_vplot(this_sample = sample_id, 
            plot_subtitle =  plot_sub,
            chr_select = chromosomes, 
            coding_only = FALSE)
```
...
if(remove_composite){lymphgen_df = dplyr::filter(lymphgen_df, LymphGen %in% c("Other", "A53", "N1", "BN2", "MCD", "ST2", "EZB", "EZB-MYC"))}
...
clin_df = data.frame(Clinical = factor(names(clin_cols), levels = rev(names(clin_cols))), hex = clin_cols) %>%
  dplyr::filter(!Clinical %in% exclude)
