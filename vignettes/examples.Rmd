...
my_metadata = my_metadata %>% select(sample_id,biopsy_id,myc_ba,cohort,pathology,lymphgen)
...
ashm_rainbow_plot(mutations_maf=my_mutations,metadata=all_metadata,bed=mybed,region=region,classification_column = "lymphgen",custom_colours = get_gambl_colours("lymphgen"))
...
gene_groups = c(rep("shared",10),c(rep("different",3)))

names(gene_groups)=bl_genes
oncoplot(blgsp_maf,writeMatrix = TRUE,genes=bl_genes,removeNonMutated = FALSE)
prettyOncoplot(maftools_obj = blgsp_maf,genes = bl_genes,
               these_samples_metadata = blgsp_metadata,
               metadataColumns = c("sex","EBV_status_inf","cohort"),
               sortByColumns = c("sex","EBV_status_inf","cohort"),
               keepGeneOrder = TRUE,
               splitGeneGroups = gene_groups,
               splitColumnName = "sex",metadataBarHeight = 10,metadataBarFontsize = 10,
