...
gene_sample_mut_freq = maf_data %>% filter(Hugo_Symbol %in% BL_genes_all) %>% 
  select(Hugo_Symbol, Tumor_Sample_Barcode) %>% unique() %>% 
  group_by(Hugo_Symbol) %>% tally() %>% 
  mutate(frequency= 100 * n / 222)

#which genes are mutated in at least 10% of patients?
gene_sample_mut_freq %>% filter(frequency >=10)

top_BL_genes = gene_sample_mut_freq %>% filter(frequency >=10) %>% pull(Hugo_Symbol)

oncoplot(maf,clinicalFeatures = c("sex","cohort"),sortByAnnotation = TRUE,genes=top_BL_genes)
