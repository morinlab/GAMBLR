...
\tiny
```{r nfkbiz, eval=F, echo=T, fig.height=4}
...
\tiny
```{r coding_ssm, echo=TRUE, message=FALSE,warning=FALSE,eval=FALSE}
...
some_genes = unique(c("IRF4","BEST3","CREB3L2","HEY2","TNFRSF13B","FCRLB","TOX",
                     "SERPINA9","MAPK10","PDGFD","PDE4B","SUGCT","SLC1A1","MYC"))
...
#scale_to_path <- function(x, data, pathology){
#  path_x = data[data$pathology == pathology, ][[cur_column()]]
#  quants <- unname(quantile(path_x, probs = c(0.05, 0.95)))
...
prettyOncoplot(maftools_obj = maf_obj,genes = genes,
               ...
               metadataColumns = c("pathology","COO_consensus","EBV_status_inf"),
               sortByColumns = c("pathology","IRF4","COO_consensus"),
               expressionColumns = c("IRF4","sbs9","sbs8","sbs1"),
               #verbose=TRUE,
