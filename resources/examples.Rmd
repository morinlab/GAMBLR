...
WARNING! This function have been updated to read from the deblacklisted and merged maf (instead of using the database). Thus, this chunk is rather slow and consumes a lot of memory. Should be updated to read from demo-dataset, when available. Alternatively, this chunk could be removed until then.
...
copy_number_vaf_plot(this_sample_id = my_sample)
...
copy_number_vaf_plot(this_sample_id = my_sample, coding_only = TRUE, genes_to_label = my_genes)
```
...
ashm_multi_rainbow_plot(regions_to_display = c("BCL2-TSS", "MYC-TSS", "SGK1-TSS", "IGL"), custom_colours = lymphgen_colours, seq_type = "genome")
```
...
prettyOncoplot(maftools_obj = maf,
               genes = genes,
               these_samples_metadata = all_metadata,
